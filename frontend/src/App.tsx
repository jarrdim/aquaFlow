...
function reconnectionPaidAtLabel(value: unknown) {
...
function Shell({ children }: { children: React.ReactNode }) {
  ...
  async function confirmReviewedReconnections() {
    ...
    const failures: Array<{ key: string; accountNumber: string; message: string }> = [];
    for (const item of selected) {
      try {
        await api.createDirectMeterReconnection({
          ...
          reason: "Physical reconnection confirmed from settled-balance eligibility review",
        ...
        failures.push({
          key: `${item.accountId}:${item.meterId}`,
    ...
    setSelectedReconnections(failures.map((failure) => failure.key));
    setReconnectionReviewConfirmed(false);
    ...
    if (failures.length) {
      ...
      for (const failure of failures) grouped.set(failure.message, [...(grouped.get(failure.message) ?? []), failure.accountNumber]);
      const summary = Array.from(grouped.entries()).map(([message, accounts]) =>
        `${message} (${accounts.slice(0, 5).join(", ")}${accounts.length > 5 ? ` and ${accounts.length - 5} more` : ""})`,
      ).join(" ");
      setReconnectionError(`${failures.length} reconnection${failures.length === 1 ? "" : "s"} failed. ${summary}`);
    }
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="eligible-reconnections-title" onMouseDown={(event) => { if (event.target === event.currentTarget && !reconnectionSubmitting) setReconnectionModalOpen(false); }}>
          <div className="flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
            <div className="flex items-start justify-between gap-4 border-b border-slate-200 px-5 py-4">
              <div><p className="text-xs font-bold uppercase tracking-wider text-emerald-700">Review and confirm</p><h2 id="eligible-reconnections-title" className="mt-1 text-xl font-extrabold text-slate-900">Settled-balance disconnected meters</h2><p className="mt-1 text-sm text-slate-500">Only disconnected accounts and meters with a zero balance or overpayment, plus verified settlement of the disconnection-time fee, are listed.</p></div>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-5">
              ...
              </> : !reconnectionError && <div className="py-16 text-center"><div className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-emerald-50 text-emerald-700">✓</div><h3 className="mt-3 font-bold text-slate-800">No eligible meters found</h3><p className="mt-1 text-sm text-slate-500">There are no zero-balance or overpaid disconnected meters ready for direct reconnection.</p></div>}
            </div>
      ...
      <div className="app-main-column flex h-screen min-w-0 flex-1 flex-col overflow-hidden">
        ...
        <header className="app-topbar h-14 bg-white border-b border-slate-200 flex items-center gap-3 px-4 flex-shrink-0">
          ...
          <div className="ml-auto flex items-center gap-1">
            {canReviewDirectReconnections && <button type="button" title="Review settled-balance disconnected meters" onClick={() => void openReconnectionReview()} className="mr-1 inline-flex items-center gap-1.5 rounded-lg bg-emerald-50 px-3 py-1.5 text-xs font-bold text-emerald-700 ring-1 ring-emerald-200 transition hover:bg-emerald-100"><span aria-hidden="true">↻</span><span className="hidden lg:inline">Reconnect check</span></button>}
            <button
              type="button"
