...
function Alert({
  ...
  return (
    <SweetAlertToast message={children} type={success ? "success" : "error"} />
  );
}
  ...
  return (
    ...
    >
        ...
        <div className="grid gap-3 md:grid-cols-3">
          <Field label="Report date">
            <input
              ...
              value={filters.asOf}
              onChange={(event) =>
                setFilters({ ...filters, asOf: event.target.value })
              }
            />
          </Field>
          <Field label="Zone">
          ...
          <Field label="Customer category">
            <CheckboxMultiSelect
              ...
              disabled={loadingLookups}
              placeholder={
                loadingLookups ? "Loading categories…" : "All categories"
              }
              value={filters.categoryIds}
              onChange={(values) =>
                setFilters({ ...filters, categoryIds: values })
              }
      ...
      <div className="relative min-h-[360px]" aria-busy={loadingDashboard}>
        ...
        <div
          className={loadingDashboard ? "pointer-events-none opacity-50" : ""}
        >
                  ...
                  <div key={label}>
                    ...
                    <div className="h-3 overflow-hidden rounded-full bg-slate-100">
                      <div
                        ...
                        style={{
                          width: `${Math.max(Number(value) ? 3 : 0, (Number(value) / max) * 100)}%`,
                        }}
                      />
                    </div>
                  </div>
        ...
        <div className="grid gap-3 md:grid-cols-3 xl:grid-cols-6">
          ...
          <Field label="Zone">
            ...
            >
              ...
              {zones.map((row) => (
                <option key={row.zoneId} value={row.zoneId}>
                  {row.zoneName}
                </option>
              ))}
            </SearchableSelect>
              ...
              {!rows.length && (
                <tr>
                  <td colSpan={7} className="p-10 text-center text-slate-400">
                    ...
                    only becomes arrears after its bill due date.
                  </td>
                </tr>
              )}
            </tbody>
    ...
    <Page
      ...
      actions={
        account && (
          <>
            <LinkButton
              to={`/billing/statements?accountId=${account.accountId}`}
              tone="slate"
            >
              View statement
            </LinkButton>
            <LinkButton to={`/arrears/notices?accountId=${account.accountId}`}>
              Demand notice
            </LinkButton>
            <LinkButton
              to={`/arrears/plans?accountId=${account.accountId}`}
              tone="green"
            >
              Payment plan
            </LinkButton>
        ...
        <>
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
              ...
              <div>
                ...
                <div className="font-semibold">
                  {account.property?.zone?.zoneName ?? "—"}
                </div>
              </div>
              <div>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <Kpi
              ...
              value={money(summary.arrearsBalance)}
              color="text-red-700"
            />
            <Kpi
              label="Current bill balance"
              value={money(summary.currentBillBalance)}
            />
            <Kpi
              label="Oldest debt"
              value={`${summary.ageDays ?? 0} days`}
              color="text-orange-600"
            />
          </div>
            ...
            <Card title="Payment arrangements">
              <div className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <Badge value={account.paymentPlans?.[0]?.status ?? "NONE"} />
                </div>
                <div className="flex justify-between">
                  <span>Promise to pay</span>
                  <Badge value={account.promisesToPay?.[0]?.status ?? "NONE"} />
                </div>
                <div className="flex justify-between">
                  <span>Last payment</span>
                  <strong>{money(summary.lastPayment?.amount)}</strong>
                </div>
                <div className="text-sm text-slate-500">
                  {date(summary.lastPayment?.paymentDate)}
                </div>
              </div>
            </Card>
            ...
            <Card title="Recovery action history">
              <div className="max-h-80 space-y-2 overflow-y-auto">
                {(account.arrearsActions ?? []).map((event: Row) => (
                  <div
                    key={event.arrearsActionId}
                    className="border-l-2 border-aqua-600 pl-3"
                  >
...
export function PaymentReminders() {
  ...
  async function send(event: FormEvent) {
    ...
    try {
      ...
      setMessage(
        `${result.queued} notification(s) queued for ${result.accounts} account(s). Open the notification queue to process delivery.`,
      );
    } catch (e: any) {
      setError(e.message);
    }
  ...
  return (
    ...
    >
      ...
      <form
        onSubmit={send}
        className="grid items-start gap-5 xl:grid-cols-[430px_minmax(0,1fr)]"
      >
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm xl:sticky xl:top-24">
          <div className="flex items-center gap-3 border-b border-slate-100 bg-gradient-to-r from-emerald-50 to-white px-5 py-4">
            <span className="grid h-10 w-10 place-items-center rounded-xl bg-emerald-600 text-lg text-white shadow-sm">
              ✉
            </span>
            <div>
              <h2 className="font-bold text-slate-900">Reminder campaign</h2>
              <p className="text-xs text-slate-500">
                Define the audience rule and delivery message.
              </p>
            </div>
          </div>
          <div className="space-y-4 p-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Minimum age (days)">
                <input
                  ...
                  min="1"
                  className={INPUT}
                  value={form.minimumAgeDays}
                  onChange={(e) =>
                    setForm({ ...form, minimumAgeDays: e.target.value })
                  }
                />
              </Field>
              <Field label="Minimum balance">
              ...
              <div className="grid grid-cols-3 gap-2">
                {(["sms", "email", "push"] as const).map((channel) => (
                  <label
                    ...
                    className={`flex items-center justify-center gap-2 rounded-xl border px-3 py-2.5 text-xs font-bold transition ${channel !== "sms" ? "cursor-not-allowed border-slate-200 bg-slate-100 text-slate-400 opacity-70" : form[channel] ? "cursor-pointer border-emerald-300 bg-emerald-50 text-emerald-800 shadow-sm" : "cursor-pointer border-slate-200 bg-white text-slate-500 hover:border-slate-300"}`}
                  >
                    <input
                      type="checkbox"
                      disabled={channel !== "sms"}
                      className="h-4 w-4 rounded border-slate-300 accent-emerald-600 disabled:cursor-not-allowed"
                      checked={channel === "sms" ? form[channel] : false}
                      onChange={(e) =>
                        setForm({ ...form, [channel]: e.target.checked })
                      }
                    />
                    {channel.toUpperCase()}
                  </label>
            ...
            <Field label="Reminder message">
              <textarea
                className={`${INPUT} min-h-40 resize-y leading-6`}
                value={form.body}
                onChange={(e) => setForm({ ...form, body: e.target.value })}
              />
              <div className="mt-1 text-right text-[11px] text-slate-400">
                {String(form.body).length} characters
              </div>
            </Field>
            <div className="rounded-xl border border-sky-100 bg-sky-50/70 p-3 text-xs text-sky-800">
              <div className="mb-2 font-bold">Personalisation fields</div>
              <div className="flex flex-wrap gap-1.5">
                {["{{customerName}}", "{{accountNumber}}", "{{balance}}"].map(
                  (field) => (
                    <code
                      key={field}
                      className="rounded-md bg-white px-2 py-1 font-semibold shadow-sm ring-1 ring-sky-100"
                    >
                      {field}
                    </code>
                  ),
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
            <div>
              <h2 className="font-bold text-slate-900">Eligible customers</h2>
              <p className="mt-0.5 text-xs text-slate-500">
                Select the accounts that should receive this campaign.
              </p>
            </div>
            {rows.length > 0 && (
              <button
                ...
                onClick={() =>
                  setSelected(
                    selected.length === rows.length
                      ? []
                      : rows.map((row) => String(row.accountId)),
                  )
                }
                className="rounded-lg bg-slate-50 px-3 py-2 text-xs font-bold text-aqua-700 transition hover:bg-aqua-50"
              >
                {selected.length === rows.length
                  ? "Clear selection"
                  : "Select all"}
          ...
          <div className="max-h-[620px] overflow-auto">
            <table className="w-full min-w-[700px]">
              <thead className="sticky top-0 z-10 bg-slate-50">
                <tr>
                  <th className={TH}>
                    <input
                      type="checkbox"
                      className="h-4 w-4 rounded border-slate-300 accent-aqua-700"
                      checked={
              ...
              <tbody>
                {rows.map((row) => (
                  <tr
                    onClick={() => toggle(String(row.accountId))}
                    className={`cursor-pointer border-t transition hover:bg-sky-50/60 ${selected.includes(String(row.accountId)) ? "bg-sky-50" : ""}`}
                    key={row.accountId}
                  >
                    <td
                      className={TD}
                    ...
                    >
                      <input
                        ...
                        onChange={() => toggle(String(row.accountId))}
                      />
                    </td>
                    <td className={TD}>
                  ...
                  <tr>
                    <td colSpan={4} className="p-0">
                      <div className="flex min-h-80 flex-col items-center justify-center px-6 text-center">
                        <span className="grid h-14 w-14 place-items-center rounded-2xl bg-emerald-50 text-2xl text-emerald-700">
                          ✓
                        </span>
                        <h3 className="mt-4 font-bold text-slate-700">
                          No customers match this rule
                        </h3>
                        <p className="mt-1 max-w-sm text-sm text-slate-400">
                          Adjust the minimum age, balance, or arrears date to
                          broaden the reminder audience.
                        </p>
                      </div>
                    </td>
                  </tr>
  ...
  const recipientAccounts = useMemo(
    () =>
      accounts.filter((account) =>
        recipientIds.includes(String(account.accountId)),
      ),
    ...
    setRecipientIds((current) =>
      current.filter((accountId) => {
        ...
        return (
          !!account &&
          (!nextZoneIds.length ||
            nextZoneIds.includes(String(account.zone?.zoneId))) &&
          (!nextCategoryIds.length ||
            nextCategoryIds.includes(String(account.category?.categoryId)))
        );
      }),
    );
  }
      ...
      <div className="grid gap-4 xl:grid-cols-[.85fr_1.15fr]">
          ...
          <form onSubmit={submit} className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <Field label="Zones">
                <CheckboxMultiSelect
                  ...
                  value={zoneIds}
                  onChange={(values) =>
                    updateAudienceFilters(values, categoryIds)
                  }
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Notice type *">
                ...
                >
                  ...
                  <option value="DISCONNECTION_NOTICE">
            ...
            <Field label="Delivery channel">
              ...
              >
                ...
                <option value="SMS_PDF">SMS + PDF</option>
              </SearchableSelect>
            </Field>
            ...
            {previewMessage && (
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm text-slate-600">
                <strong className="block text-slate-800">
                  Preview for {previewAccount.customerName}
                </strong>
                {previewMessage}
              </div>
            )}
        ...
        <div className="space-y-4">
          <Card title={`${total} notice(s) in register`}>
            <form
              className="mb-3 grid gap-2 md:grid-cols-[1fr_220px_auto]"
              onSubmit={(event) => {
                event.preventDefault();
                void load(1);
              }}
            >
              <input
                ...
                placeholder="Search notice, account or customer"
              />
              <SearchableSelect
                className={INPUT}
                value={status}
                          ...
                          >
                            ...
                            <td className={TD}>
                              ...
                              <div className="text-xs">
                                {pretty(row.noticeType)}
                              </div>
                            </td>
                            <td className={TD}>
                              {row.account?.accountNumber}
                              <div className="text-xs">
                                {customerName(row.account?.customer)}
                              </div>
                            </td>
                            <td className={TD}>
...
export function PaymentPlans() {
  ...
  const load = () =>
    Promise.all([api.listPaymentPlans(), api.listArrearsAccounts()])
      .then(([a, b]) => {
        setRows(a);
        setAccounts(b);
  ...
  const account = accounts.find(
    (row) => String(row.accountId) === form.accountId,
  );
  const remaining = Math.max(
    0,
    Number(account?.currentBalance ?? 0) - Number(form.depositAmount || 0),
  );
  const installment = form.numberOfInstallments
    ? remaining / Number(form.numberOfInstallments)
    : 0;
  async function submit(e: FormEvent) {
    e.preventDefault();
    try {
      await api.createPaymentPlan(form);
      setMessage("Payment plan proposed for approval.");
      await load();
    } catch (value: any) {
  ...
  async function decide(decision: "APPROVE" | "REJECT" | "RETURN") {
    if (!selected) return;
    try {
      await api.decidePaymentPlan(selected.paymentPlanId, decision, comments);
      setMessage(`Payment plan ${decision.toLowerCase()}d.`);
      setComments("");
      await load();
    } catch (value: any) {
      setError(value.message);
  ...
  return (
    <Page
      ...
      subtitle="Create structured instalment arrangements and monitor compliance"
    >
      {error && <Alert>{error}</Alert>}
      {message && <Alert success>{message}</Alert>}
          ...
          <form onSubmit={submit} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-xl bg-slate-50 p-3">
                <div className="text-sm text-slate-500">Outstanding debt</div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="Number of instalments">
                <input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, numberOfInstallments: e.target.value })
                  }
                />
              </Field>
            </div>
            <div className="rounded-xl bg-emerald-50 p-3 text-emerald-800">
              Estimated instalment: <strong>{money(installment)}</strong>
            </div>
            <div className="grid grid-cols-2 gap-3">
            ...
            <Button type="submit" className="w-full" disabled={!form.accountId}>
              Generate and submit plan
            </Button>
          </form>
        </Card>
        <div className="space-y-4">
          <Card title={`${rows.length} payment plan(s)`}>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[760px]">
                <thead>
                  <tr>
                    <th className={TH}>Plan / Customer</th>
                    <th className={TH}>Debt</th>
                    <th className={TH}>Instalments</th>
                ...
                <tbody>
                  {rows.map((row) => (
                    <tr key={row.paymentPlanId} className="border-t">
                      <td className={TD}>
                        <strong>{row.planReference}</strong>
                        <div className="text-xs">
                          {row.account?.accountNumber} ·{" "}
                          {customerName(row.account?.customer)}
                        </div>
                      </td>
                      <td className={TD}>{money(row.totalDebt)}</td>
                      <td className={TD}>
                        {row.numberOfInstallments} ×{" "}
                        {money(row.installmentAmount)}
                      </td>
                      <td className={TD}>
          ...
          {selected && (
            <Card title={`Plan tracking · ${selected.planReference}`}>
              <div className="mb-3 grid gap-3 sm:grid-cols-3">
                <Kpi label="Original debt" value={money(selected.totalDebt)} />
                <Kpi label="Deposit" value={money(selected.depositAmount)} />
                <Kpi
                  label="Scheduled balance"
                  value={money(
                    Number(selected.totalDebt) - Number(selected.depositAmount),
                  )}
                />
              </div>
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {(selected.installments ?? []).map((item: Row) => (
                      <tr key={item.installmentId} className="border-t">
                        <td className={TD}>{item.installmentNumber}</td>
                        <td className={TD}>{date(item.dueDate)}</td>
                        <td className={TD}>{money(item.amountDue)}</td>
                        <td className={TD}>{money(item.amountPaid)}</td>
                        <td className={TD}>
...
export function PromisesToPay() {
  ...
  const load = () =>
    Promise.all([api.listPromisesToPay(status), api.listArrearsAccounts()])
      .then(([a, b]) => {
        setRows(a);
        setAccounts(b);
      })
  ...
  async function submit(e: FormEvent) {
    e.preventDefault();
    try {
      await api.createPromiseToPay(form);
  ...
  async function update(id: string, value: "KEPT" | "BROKEN" | "CANCELLED") {
    try {
      await api.updatePromiseStatus(id, value);
      setMessage(`Promise marked ${value.toLowerCase()}.`);
      await load();
    } catch (e: any) {
      setError(e.message);
    }
  ...
  return (
    <Page
      ...
      subtitle="Record customer commitments and follow up honoured or broken promises"
    >
      {error && <Alert>{error}</Alert>}
      {message && <Alert success>{message}</Alert>}
      <div className="grid items-start gap-5 xl:grid-cols-[430px_minmax(0,1fr)]">
        <Card
          ...
          className="xl:sticky xl:top-24 overflow-hidden"
        >
          <div className="mb-4 flex items-start gap-3 rounded-xl border border-emerald-100 bg-emerald-50/70 p-3">
            <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-emerald-600 font-bold text-white">
              ✓
            </span>
            <div>
              ...
              <p className="mt-0.5 text-xs leading-5 text-emerald-800">
                Record the agreed amount, due date and contact evidence for
                follow-up.
              </p>
            </div>
          </div>
          <form onSubmit={submit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Promised payment date *">
                <input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, expectedPaymentDate: e.target.value })
                  }
                />
              </Field>
            ...
            <Button type="submit" tone="green" className="w-full">
              Save promise
            </Button>
          </form>
        </Card>
        <Card title="Promise tracking" className="overflow-hidden">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-slate-50 p-3">
            <div>
              ...
              <div className="text-xs text-slate-500">
                Review and update the outcome of customer promises.
              </div>
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[850px]">
              <thead>
                <tr>
                  <th className={TH}>Reference / Customer</th>
                  <th className={TH}>Amount</th>
                  <th className={TH}>Due date</th>
              ...
              <tbody>
                {rows.map((row) => (
                  <tr
                    className="border-t transition hover:bg-sky-50/60"
                    key={row.promiseId}
                  >
                    <td className={TD}>
                      <strong>{row.promiseReference}</strong>
                      <div className="text-xs">
                        {row.account?.accountNumber} ·{" "}
                        {customerName(row.account?.customer)}
                      </div>
                    </td>
                    <td className={TD}>{money(row.promisedAmount)}</td>
                    <td className={TD}>{date(row.expectedPaymentDate)}</td>
                    <td className={TD}>{pretty(row.contactMethod)}</td>
                    <td className={TD}>
                      <Badge value={row.status} />
                    </td>
                    <td className={TD}>
                      {row.status === "OPEN" ? (
                        <div className="flex gap-2">
                          <button
                            className="rounded-lg bg-emerald-50 px-2.5 py-1.5 text-xs font-bold text-emerald-700 transition hover:bg-emerald-100"
                            onClick={() => update(row.promiseId, "KEPT")}
                          >
                            Kept
                          </button>
                          <button
                            className="rounded-lg bg-red-50 px-2.5 py-1.5 text-xs font-bold text-red-700 transition hover:bg-red-100"
                            onClick={() => update(row.promiseId, "BROKEN")}
                          >
                            Broken
                          </button>
                          <button
                            className="rounded-lg px-2.5 py-1.5 text-xs font-bold text-slate-500 transition hover:bg-slate-100"
                            onClick={() => update(row.promiseId, "CANCELLED")}
                          >
                            Cancel
                          </button>
                        </div>
                    ...
                    <td colSpan={6} className="p-0">
                      <div className="flex min-h-72 flex-col items-center justify-center px-6 text-center">
                        ...
                        <p className="mt-1 text-sm text-slate-400">
                          New customer commitments will appear here for
                          follow-up.
                        </p>
                      </div>
                    </td>
...
export function DisconnectionLists() {
    ...
    try {
      ...
      setReview(
        (current) =>
          listRows.find(
            (row: Row) =>
              row.disconnectionListId === current?.disconnectionListId,
          ) ?? listRows[0],
      );
      setSelected((values) =>
        values.filter((value) =>
          eligibleRows.some((row: Row) => String(row.accountId) === value),
        ),
      );
    } catch (e: any) {
      setError(e.message);
  ...
  async function create() {
    try {
      setAction("CREATE");
      setError("");
      await api.createDisconnectionList({
        accountIds: selected,
  ...
  async function decide(decision: "APPROVE" | "REJECT" | "RETURN") {
    ...
    try {
      setAction(decision);
      setError("");
      await api.decideDisconnectionList(
        review.disconnectionListId,
        decision,
        comments,
      );
      setMessage(`Disconnection list ${decision.toLowerCase()}d.`);
      setComments("");
      await load();
    } catch (e: any) {
      setError(e.message);
        ...
        <div className="mb-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <Field label="Minimum arrears age (days)">
            <input
              ...
              onChange={(e) =>
                setFilters({ ...filters, minimumAgeDays: e.target.value })
              }
            />
          </Field>
          <Field label="Minimum balance">
                    ...
                    <tr className="border-t" key={row.accountId}>
                      <td className={TD}>
                        <input
                          ...
                          onChange={() =>
                            setSelected((values) =>
                              values.includes(String(row.accountId))
                                ? values.filter(
                                    (value) => value !== String(row.accountId),
                                  )
        ...
        <Card title="Approval decision">
          {loading ? (
            ...
            <>
              ...
              <div className="my-3 max-h-44 overflow-y-auto">
...
export function DebtWriteOffs() {
  ...
  const load = () =>
    Promise.all([
      api.listDebtWriteOffs(),
      api.listArrearsAccounts({ minimumAgeDays: "120" }),
    ])
      .then(([a, b]) => {
        setRows(a);
        setAccounts(b);
  ...
  async function submit(e: FormEvent) {
    e.preventDefault();
    try {
      await api.createDebtWriteOff(form);
      setMessage("Write-off request submitted for Finance Manager decision.");
      await load();
    } catch (e: any) {
      setError(e.message);
    }
  ...
  async function decide(decision: "APPROVE" | "REJECT" | "RETURN") {
    if (!selected) return;
    try {
      await api.decideDebtWriteOff(selected.writeOffId, decision, comments);
      setMessage(`Write-off request ${decision.toLowerCase()}d.`);
      setComments("");
      await load();
    } catch (e: any) {
      setError(e.message);
  ...
  return (
    <Page
      title="Debt write-off control"
      subtitle="Request and independently approve debts considered unrecoverable"
    >
      {error && <Alert>{error}</Alert>}
      {message && <Alert success>{message}</Alert>}
      <div className="grid items-start gap-5 xl:grid-cols-[450px_minmax(0,1fr)]">
        <Card
          title="Write-off request"
          className="xl:sticky xl:top-24 overflow-hidden"
        >
          <div className="mb-4 flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-3">
            <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-amber-500 font-bold text-white">
              !
            </span>
            <div>
              ...
              <p className="mt-0.5 text-xs leading-5 text-amber-800">
                Document all recovery attempts. Submission requires an
                independent approval decision.
              </p>
            </div>
          </div>
          <form onSubmit={submit} className="space-y-4">
            <Field label="Customer account (120+ days) *">
              <AccountSelect
                rows={accounts}
                value={form.accountId}
                onChange={(value) => {
                  const row = accounts.find(
                    (item) => String(item.accountId) === value,
                  );
                  setForm({
                    ...form,
                    accountId: value,
                    amount: row?.arrearsBalance ?? "",
                    debtAgeDays: row?.ageDays ?? 120,
                  });
                }}
              />
            </Field>
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="Debt age days">
                <input
                  type="number"
                  min="120"
                  className={INPUT}
                  value={form.debtAgeDays}
                  onChange={(e) =>
                    setForm({ ...form, debtAgeDays: e.target.value })
                  }
                />
              </Field>
            </div>
            <Field label="Recovery actions taken *">
              <textarea
                className={`${INPUT} min-h-24`}
                placeholder="Reminders, demand notices, disconnection and follow-up actions"
                value={form.recoveryActions}
                onChange={(e) =>
                  setForm({ ...form, recoveryActions: e.target.value })
                }
              />
            </Field>
        ...
        <div className="space-y-4">
          <Card title="Write-off request register" className="overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full min-w-[700px]">
                <thead>
                  <tr>
                ...
                <tbody>
                  {rows.map((row) => (
                    <tr
                      onClick={() => setSelected(row)}
                      className={`cursor-pointer border-t transition hover:bg-sky-50/60 ${selected?.writeOffId === row.writeOffId ? "bg-sky-50" : ""}`}
                      key={row.writeOffId}
                    >
                      <td className={TD}>
                        <strong>{row.writeOffReference}</strong>
                        <div className="text-xs">
                          {row.account?.accountNumber} ·{" "}
                          {customerName(row.account?.customer)}
                        </div>
                      ...
                      <td className={TD}>{row.debtAgeDays} days</td>
                      <td className={TD}>
                        <Badge value={row.status} />
                      </td>
                      <td className={TD}>
                        <button
                          className="rounded-lg bg-aqua-50 px-2.5 py-1.5 text-xs font-bold text-aqua-700 transition hover:bg-aqua-100"
                          onClick={() => setSelected(row)}
                        >
                          Review
                        </button>
                    ...
                    <tr>
                      ...
                      >
                        <div className="flex min-h-40 flex-col items-center justify-center">
                          <span className="grid h-12 w-12 place-items-center rounded-2xl bg-slate-100 text-xl text-slate-500">
                            ◇
                          </span>
                          <div className="mt-3 font-bold text-slate-600">
                            No write-off requests
                          </div>
          ...
          {selected && (
            <Card title={`Approval decision · ${selected.writeOffReference}`}>
              <div className="rounded-xl bg-slate-50 p-3">
                ...
                <div className="mt-1 text-sm">
                  <strong>Recovery history:</strong> {selected.recoveryActions}
                </div>
              </div>
              <Field label="Decision comments *">
                <textarea
              ...
              <div className="mt-3 flex justify-end gap-2">
                <Button
                  tone="red"
                  disabled={
                    comments.length < 3 || selected.status !== "PENDING"
                  }
                  onClick={() => decide("REJECT")}
                >
                  Reject
...
export function DebtRecoveryReport() {
  ...
  const exportRows = (data?.rows ?? []).map((row: Row) => ({
    ...
    Customer: row.customerName,
    Zone: row.zone?.zoneName ?? "",
    Balance: row.arrearsBalance,
    "Age days": row.ageDays,
  }));
  return (
    <Page
      title="Debt recovery report"
      subtitle="Opening debt, new arrears, recoveries, write-offs and closing position"
      actions={
        <>
          <Button
            tone="slate"
    ...
    >
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
        <Kpi label="Opening arrears" value={money(data?.openingArrears)} />
        <Kpi
          label="New arrears"
          value={money(data?.newArrears)}
          color="text-orange-600"
        />
        <Kpi
          label="Amount recovered"
          value={money(data?.amountRecovered)}
          color="text-emerald-700"
        />
        <Kpi
          label="Written off"
          value={money(data?.writtenOff)}
          color="text-red-600"
        />
        <Kpi
          label="Closing arrears"
          value={money(data?.closingArrears)}
          color="text-red-700"
        />
        <Kpi
          label="Recovery rate"
          value={`${Number(data?.recoveryRate ?? 0).toFixed(2)}%`}
          color="text-aqua-700"
        />
      </div>
      <Card title="Closing arrears detail" className="mt-4">
        <div className="overflow-x-auto">
              ...
              {!data?.rows?.length && (
                <tr>
                  <td colSpan={4} className="p-8 text-center text-slate-400">
                    No closing arrears for this report date.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <Button
          ...
          onClick={() =>
            exportExcel(
              ...
              "Arrears audit",
              rows.map((row) => ({
                Date: dateTime(row.createdAt),
                Account: row.account?.accountNumber ?? "",
                Customer: customerName(row.account?.customer),
                Action: pretty(row.actionType),
                Details: row.details,
                User: row.performer
                  ? `${row.performer.firstName} ${row.performer.lastName}`
                  : "System",
              })),
            )
          }
      ...
      <Card title={`${rows.length} audit event(s)`}>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[900px]">
            ...
            <tbody>
              {rows.map((row) => (
                <tr className="border-t" key={row.arrearsActionId}>
