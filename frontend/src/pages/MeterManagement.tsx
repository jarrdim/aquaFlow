...
export function DirectMeterService() {
  ...
  async function selectHistoryAccount(row: AnyRecord) {
    const accountNumber = String(row.accountNumber ?? "");
    setSearch(accountNumber);
    setLoading(true);
    setError("");
    try {
      const options = await api.getDirectMeterServiceOptions(accountNumber);
      const nextItems = options.items ?? [];
      const match = nextItems.find((item: AnyRecord) => String(item.accountId) === String(row.accountId))
        ?? nextItems.find((item: AnyRecord) => String(item.meterNumber) === String(row.metadata?.meterNumber));
      setItems(nextItems);
      setSelected(match ?? null);
      if (!match) setError(`No current customer meter is available for account ${accountNumber}.`);
    } catch (err: any) { setError(err.message); }
    finally { setLoading(false); }
  }
        ...
        <Card title="Recent direct actions">
          ...
          {historyLoading ? <Spinner /> : <>
            <div className="overflow-x-auto"><table className="min-w-full"><thead><tr className="border-b border-slate-100"><th className={TH}>Date</th><th className={TH}>Action</th><th className={TH}>Account / customer</th><th className={TH}>Meter</th><th className={TH}>Payment</th><th className={TH}>Recorded by</th></tr></thead>
              <tbody>{historyItems.map((row) => <tr key={row.actionId} className="border-b border-slate-50"><td className={TD}>{new Date(row.createdAt).toLocaleString()}</td><td className={TD}><Status value={row.actionType.includes("RECONNECTION") ? "ACTIVE" : "DISCONNECTED"} /></td><td className={TD}><button type="button" className="text-left hover:text-aqua-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-aqua-500" onClick={() => void selectHistoryAccount(row)}><strong className="text-aqua-800 underline decoration-aqua-300 underline-offset-2">{row.accountNumber}</strong><span className="block text-xs text-slate-400">{row.customerName}</span></button></td><td className={TD}>{row.metadata?.meterNumber ?? "—"}</td><td className={TD}><div className="flex flex-col items-start gap-1"><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ring-inset ${row.paid ? "bg-emerald-50 text-emerald-700 ring-emerald-600/20" : "bg-red-50 text-red-700 ring-red-600/20"}`}>{row.paid ? "Paid" : "Not paid"}</span>{Number(row.accountCreditAvailable ?? 0) > 0 && <span className="inline-flex rounded-full bg-violet-50 px-2.5 py-1 text-xs font-semibold text-violet-700 ring-1 ring-inset ring-violet-600/20">Overpayment KSh {money(row.accountCreditAvailable)}</span>}</div></td><td className={TD}>{row.performedByName ?? "System"}</td></tr>)}</tbody>
