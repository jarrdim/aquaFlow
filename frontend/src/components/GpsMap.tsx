...
export function GpsMap({ latitude, longitude, label = "Captured location", className = "", empty = false }: GpsMapProps) {
  ...
  return <div className={`overflow-hidden rounded-xl border border-slate-200 bg-slate-50 shadow-sm ${className}`}><iframe title={`${label} map`} src={mapUrl} className="h-44 w-full border-0" loading="lazy" /><div className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-200 bg-white px-3 py-2.5"><div><div className="text-xs font-semibold text-slate-700">{label}</div><div className="font-mono text-[11px] text-slate-500">{lat.toFixed(6)}, {lng.toFixed(6)}</div></div><a href={`https://www.openstreetmap.org/?mlat=${lat}&mlon=${lng}#map=17/${lat}/${lng}`} target="_blank" rel="noreferrer" className="text-xs font-bold text-emerald-700 transition hover:text-emerald-900">Open map</a></div></div>;
}
